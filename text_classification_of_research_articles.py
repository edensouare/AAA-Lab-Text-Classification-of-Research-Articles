# -*- coding: utf-8 -*-
"""Text Classification of Research Articles

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1H0PYhG-cRXdjCjBQrDfbs8D5d61LEUGM
...
### Subtask:
Cleaning and preprocessing the text data. Removing special characters, numbers, stop words, and performing lemmatization.
"""
...
### Subtask:
Convert the text data into numerical features that can be used by the machine learning model. Using TfidfVectorizer
"""
...
### Subtask:
Choosing a suitable machine learning model for text classification (like Logistic Regression, Naive Bayes, or SVM) and train it on the training data
"""
...
### Subtask:
Evaluating the performance of the trained model on the testing data using metrics (accuracy, precision, recall, F1-score).
"""
...
### Subtask:
Create a function to filter articles based on keywords using the trained model
"""
...
"""# MAIN Task
Now I'm going to refine the text classification model

## Explore different models

### Subtask:
Trying to train and evaluate other text classification models to improve accuracy, such as Logistic Regression, Support Vector Machines (SVM), and RandomForestClassifier (used in https://www.analyticsvidhya.com/blog/2021/12/text-classification-of-news-articles/)
"""
...
### Subtask:
For the most promising models (Random Forest and SVC), perform hyperparameter tuning to find the optimal settings that maximize performance.
...
### Subtask:
Retrain the selected best model (RandomForestClassifier) using the optimal hyperparameters on the entire dataset (training and testing combined) to utilize all available data for the final model.
...
### Subtask:
Update the filtering step to use the best-performing model (`final_model`) to identify relevant articles from the entire dataset.
...
### Subtask:
Display a sample of articles predicted as 'useful' and 'not useful' for manual review.
"""
...
### Subtask:
Extract the articles predicted as 'useful' into a separate DataFrame and save it for further analysis.
"""
...
This notebook focuses on classifying research articles related to Artificial Intelligence (AI) and addiction. The process involves several key steps: data loading, preprocessing, feature extraction, model training and evaluation, hyperparameter tuning, selecting the best model, and finally filtering the articles based on the trained model.

## Data Loading and Initial Exploration

The notebook starts by importing necessary libraries for data manipulation, analysis, visualization, and machine learning. The dataset, "Systematic Review about AI - Duplicate check.csv", is loaded into a pandas DataFrame. Initial exploration is performed to understand the data's shape, column information, and value counts of the 'Keywords(SSCI)' column.

## Preprocessing

This stage involves cleaning and preparing the text data for analysis. The 'Title' and 'Abstract' columns are combined into a new 'text' column. The text is then converted to lowercase, and special characters and numbers are removed using regular expressions. The cleaned text is tokenized, stop words are removed using the NLTK library, and lemmatization is applied to reduce words to their base form. Finally, the tokens are joined back into a string.

## Feature Extraction

To convert the text data into a format suitable for machine learning models, the TfidfVectorizer is used. This technique calculates the Term Frequency-Inverse Document Frequency (TF-IDF) for each word, representing the importance of words in a document relative to the entire corpus. The resulting TF-IDF matrix is then used as features for the models.

## Dataset Splitting and Initial Classification

A target variable 'useful' is created based on whether the article's 'Title' and 'Abstract' contain keywords related to both AI and addiction. The dataset is then split into training and testing sets to evaluate the models' performance on unseen data.

An initial text classification model, Multinomial Naive Bayes, is trained on the training data and evaluated using accuracy, classification report, and confusion matrix.

## Exploring Different Models

To potentially improve the classification performance, other text classification models are explored: Logistic Regression, Support Vector Machines (using LinearSVC for efficiency), and RandomForestClassifier. These models are trained on the training data and evaluated using the same metrics as the initial model. The evaluation results for each model are printed to compare their performance.

## Hyperparameter Tuning

Based on the initial evaluation, RandomForestClassifier and LinearSVC show promising results. Hyperparameter tuning is performed on these models using `GridSearchCV` with a 5-fold cross-validation and 'f1_weighted' as the scoring metric. This process aims to find the optimal combination of hyperparameters that maximizes the model's performance. The best parameters and corresponding best scores for each model are printed.

## Selecting the Best Model

The best models from the hyperparameter tuning step (RandomForestClassifier and LinearSVC) are compared based on their 'f1_weighted' scores. The model with the higher score is selected as the best model for the task.

## Retraining the Best Model and Final Evaluation

The selected best model (RandomForestClassifier) is then retrained using its optimal hyperparameters on the entire training dataset to leverage all available data for the final model. The final model is evaluated on the held-out test set to assess its generalization capability. Accuracy, classification report, and confusion matrix are used for the final evaluation.

## Implementing Filtering with the Best Model

The trained final model is used to predict 'useful' articles on the entire dataset. A new column 'predicted_useful_final' is added to the DataFrame to store these predictions. The DataFrame is then filtered to keep only the articles predicted as 'useful'. The number of useful articles found and a sample of these articles are displayed.

## Manual Review of Predicted Articles

To provide a qualitative assessment of the model's performance, a sample of articles predicted as 'useful' and 'not useful' by the final model is displayed for manual review.

## Extracting and Saving Useful Articles

The articles predicted as 'useful' by the final model are extracted into a separate DataFrame called `useful_articles_final`. This DataFrame contains all the original information for these articles. The `useful_articles_final` DataFrame is then saved to a CSV file named "useful_articles_for_analysis.csv" for further analysis.

## Refined Filtering and Saving

A refined filtering step is implemented to further narrow down the 'useful' articles. This step filters out articles where "dependence" is the *only* addiction keyword present in the Abstract from the predefined list of addiction keywords. The number of articles remaining after this refined filtering is displayed, and a sample of these articles is shown. This refined list of articles is saved to "filtered_useful_articles.csv".

Additionally, the articles that were filtered out in this refined step (those containing only "dependence" as an addiction keyword in the Abstract) are also extracted into a separate DataFrame and saved to "dependence_only_articles.csv". This allows for separate analysis of this specific subset.

## Summary of Findings and Next Steps

The notebook concludes with a summary of the key findings, including the performance metrics of the models, the selection of the best model, and the number of useful articles identified. It also provides insights and suggests next steps, such as further analysis of the filtered articles to extract specific information and themes related to AI and addiction.
"""